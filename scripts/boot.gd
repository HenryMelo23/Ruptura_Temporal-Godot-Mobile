...
func _draw_temporal_core(viewport: Vector2, t: float) -> void:
	...
	draw_circle(center, radius * (0.82 + pulse * 0.06), Color(0.0, 0.80, 1.0, 0.055))
	...
	for i in range(3):
		...
		draw_arc(center, r * 0.92, -t * (0.8 + i * 0.12), -t * (0.8 + i * 0.12) + PI * 1.20, 64, Color(1.0, 0.08, 0.66, 0.30 - i * 0.06), 1.4)
...
func _draw_boot_title(viewport: Vector2, t: float) -> void:
	...
	_draw_centered_with_font(title_font, text, pos + Vector2(-2.0 + jitter, 0.0), size, Color(1.0, 0.08, 0.70, 0.42))
...
func _draw_status(viewport: Vector2, t: float) -> void:
	...
	draw_rect(rect, Color(0.010, 0.016, 0.034, 0.76), true)
	...
	if load_requested and progress <= 0.0:
		visible_progress = 0.32 + pulse * 0.10
