...
static func draw_catalog(host: Node, viewport: Vector2) -> void:
	...
	if not portrait and visible_items.size() > visible:
		...
		host._draw_big_button(prev_rect, "<", Color(0.04, 0.08, 0.10, 0.88), Color(0.42, 0.92, 1.0), false)
		host._draw_big_button(next_rect, ">", Color(0.04, 0.08, 0.10, 0.88), Color(0.42, 0.92, 1.0), false)
...
static func draw_catalog_search_and_filters(host: Node, viewport: Vector2) -> void:
	...
	host._draw_holo_panel(search_area, Color(0.0, 0.78, 1.0), false, 0.50)
	var query_text: String = host.catalog_search_query if host.catalog_search_query.strip_edges() != "" else "BUSCAR REGISTRO..."
	var query_color := Color.WHITE if host.catalog_search_query.strip_edges() != "" else Color(0.60, 0.74, 0.82, 0.72)
	...
	if host.catalog_search_query.strip_edges() != "":
		...
		host.draw_rect(clear_rect, Color(0.20, 0.04, 0.08, 0.70), true)
	...
	for mode_key in filter_modes():
		...
		var color := Color(0.0, 1.0, 0.82) if selected else Color(0.16, 0.30, 0.38)
...
static func draw_catalog_detail(host: Node, viewport: Vector2) -> void:
	...
	var text_y: float
	if portrait:
		image_rect = Rect2(panel.position + Vector2(20, 20), Vector2(panel.size.x - 40, min(190.0, panel.size.y * 0.30)))
	...
	host.draw_rect(image_rect, Color(color.r, color.g, color.b, 0.50), false, 2)
	...
	host.draw_string(host.font, Vector2(text_x + text_w - min(190.0, text_w * 0.45), text_y), source_text, HORIZONTAL_ALIGNMENT_RIGHT, min(190.0, text_w * 0.45), 11, Color(0.70, 0.86, 0.92, 0.76))
	text_y += 28
	...
	text_y += 18
	host._draw_wrapped(detail_lore(host, item), Rect2(text_x, text_y, text_w, 66.0), 13, Color(0.70, 0.82, 0.88))
	text_y += 76.0
	...
	host._draw_wrapped(detail_mechanics(host, item), Rect2(text_x, text_y, text_w, panel.end.y - text_y - 22.0 - related_area_h - note_h), 13, Color(0.82, 0.90, 0.94))
	if note_h > 0.0:
	...
	host._draw_big_button(Rect2(viewport.x * 0.5 - 120, viewport.y - 72, 240, 48), back_label, Color(0.08, 0.04, 0.10, 0.90), Color(1.0, 0.20, 0.78))


static func draw_related_entries(host: Node, item: Dictionary, area: Rect2, color: Color) -> void:
	host.draw_string(host.font, area.position + Vector2(0, 12), "RELACIONADOS", HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color(color.r, color.g, color.b, 0.90))
