...
func _run() -> void:
	...
	_check(game.qa_streaming_unlocked, "memory saver should preserve opt-in QA streaming unlock")
	_check(game.qa_streaming_enabled, "memory saver should not cancel a player-requested QA stream")
	...
	_check(game.qa_streaming_quality_mode == "720p", "QA streaming quality selector should still cycle after memory saver clamp")
