...
REGISTRADOR_RETORNO_SLOW_MS = 3000
REGISTRADOR_SLOW_INICIAL = 0.0
REGISTRADOR_ROLETAGEM_TRAVAR_EM = 0.78
REGISTRADOR_PARTICULAS_MAX = 96
REGISTRADOR_ANEL_PULSO_MAX = 420
REGISTRADOR_SHAKE_MAX = 10
...
def _rotulo_flipflop(tipo):
    return "SR" if str(tipo or "").upper() == "RS" else str(tipo or "?").upper()


def _descricao_buff_registrador(buff=None, porta=None):
    buff = str(buff or _ESTADO_LOGICO.get("registrador_buff_ativo") or "")
    porta = porta or _ESTADO_LOGICO.get("registrador_porta_capturada")
    if porta == "-":
        porta = None
    if buff in ("Q", "JK_Q", "T_Q", "RS_SET"):
        return "DANO +20% | CADENCIA +10% | +1 SALTO"
    if buff in ("Q_DEF", "JK_Q_DEF", "T_Q_DEF", "RS_RESET"):
        return "ESCUDO 1 HIT | SLOW EM AREA | RUIDO -30%"
    if buff == "D_OR":
        return "CORRENTE +1 SALTO | AREA CONDUTORA"
    if buff == "D_AND":
        return "DANO LOGICO +25%"
    if buff == "D_XOR":
        return "VELOCIDADE +10% | CADENCIA +15%"
    if buff == "D_NAND":
        return "ESCUDO 1 HIT | DEFESA LOGICA"
    if buff == "D_NOR":
        return "CAMPO DE LENTIDAO EM AREA"
    if porta:
        return f"PORTA {porta} CAPTURADA"
    return "SEM BUFF ATIVO"


def _cor_buff_registrador(buff=None):
    buff = str(buff or _ESTADO_LOGICO.get("registrador_buff_ativo") or "")
    if buff in ("Q", "JK_Q", "T_Q", "RS_SET", "D_AND", "D_OR"):
        return COR_LOGICO_1
    if buff in ("Q_DEF", "JK_Q_DEF", "T_Q_DEF", "RS_RESET", "D_NAND", "D_NOR"):
        return COR_LOGICO_0
    if buff == "D_XOR":
        return COR_LOGICO_OK
    return COR_CONDUTORA_FRIA
...
def _aplicar_buff_registrador(nome, estado_q, duracao, tempo_atual, stacks=1, porta=None, feedback=None):
    ...
    estado["registrador_pulso"] = {
        "inicio_ms": int(tempo_atual),
        "fim_ms": int(tempo_atual) + 1200,
        "buff": nome,
        "texto": _descricao_buff_registrador(nome, porta),
        "q": bool(estado_q),
        "porta": porta,
...
def _resolver_registrador_instavel(
    ...
    def aplicar_estado(nome, q, duracao, texto, stacks=1, porta=None, bonus=1.0):
        ...
        ganho = _descricao_buff_registrador(nome, porta)
        feedback(f"{texto} | {ganho}", _cor_buff_registrador(nome))
...
def _combo_roleta_registrador(tipo):
    rotulo = _rotulo_flipflop(tipo)
    return [rotulo, rotulo, rotulo]
...
def fechar_circuitos(inimigos, tempo_atual, dano_base, efeitos_texto=None, jogador_rect=None, teleporte_recente=None):
    ...
    estado["registrador_feedback"] = "Cortex em rolagem"
    estado["registrador_feedback_ms"] = agora
    _registrar_feedback(f"FLIP-FLOP {rotulo}: ???", 1, agora)
    if jogador_rect is not None and efeitos_texto is not None:
        efeitos_texto.append({
            "texto": "CASSINO CORTICAL",
            "x": jogador_rect.centerx,
            "y": jogador_rect.top - 35,
            "tempo_inicio": agora,
...
def _atualizar_roleta_registrador(inimigos, tempo_atual, dano_base, efeitos_texto=None):
    ...
    estado["registrador_feedback"] = f"Flip-Flop {_rotulo_flipflop(tipo)} resolvido"
    estado["registrador_feedback_ms"] = agora
    return mortos
...
def animacao_registrador_ativa(manifestacao, tempo_atual=None):
    if not ativa(manifestacao):
        return False
    return bool(_estado_logico().get("registrador_animacao_ativa"))
...
def fator_tempo_registrador(manifestacao, tempo_atual):
    ...
    if agora < slow_fim and slow_fim > anim_fim:
        ...
        retorno = 1.0 - (1.0 - progresso) ** 3
        return REGISTRADOR_SLOW_INICIAL + (1.0 - REGISTRADOR_SLOW_INICIAL) * retorno
    return 1.0
...
def _desenhar_hud_logico(tela, tempo_atual, perfil):
    ...
    desc_buff = _descricao_buff_registrador(buff_reg, porta_cap)
    surf.blit(fonte_peq.render(f"Buff: {buff_reg}", True, _cor_buff_registrador(buff_reg)), (112, 84))
    surf.blit(_fonte(11, False).render(desc_buff[:38], True, COR_CONDUTORA_FRIA), (12, 102))
...
def _desenhar_roleta_registrador(tela, tempo_atual, perfil):
    ...
    combo = estado.get("registrador_combo") or ["D", "D", "D"]
    opcoes = ["D", "T", "SR", "JK"]
    tipo_final = combo[1] if len(combo) > 1 else combo[0]
    fonte_titulo = _fonte(18, True)
    fonte_reel = _fonte(46, True)
    fonte_peq = _fonte(14, True)
    fonte_grande = _fonte(34, True)
    rng = random.Random(inicio + agora // 28)
    shake = int((1.0 - progresso) * REGISTRADOR_SHAKE_MAX) if progresso < 0.90 else 0

    overlay = pygame.Surface((largura_tela, altura_tela), pygame.SRCALPHA)
    alpha = 110 if perfil == "baixo" else 150
    overlay.fill((0, 0, 0, alpha))
    for i in range(36 if perfil == "baixo" else REGISTRADOR_PARTICULAS_MAX):
        px = rng.randint(0, largura_tela)
        py = rng.randint(0, altura_tela)
        raio = rng.randint(1, 4)
        cor = COR_CONDUTORA_FRIA if i % 3 else COR_CONDUTORA_CLARA
        pygame.draw.circle(overlay, (*cor, rng.randint(55, 150)), (px, py), raio)
        if perfil != "baixo" and i % 4 == 0:
            pygame.draw.line(overlay, (*COR_CONDUTORA, 85), (px, py), (px + rng.randint(-34, 34), py + rng.randint(-34, 34)), 1)
    tela.blit(overlay, (0, 0))

    titulo_txt = f"FLIP-FLOP {tipo_final} NO CORTEX" if progresso >= REGISTRADOR_ROLETAGEM_TRAVAR_EM else "CASSINO CORTICAL"
    titulo = fonte_titulo.render(titulo_txt, True, COR_CONDUTORA_FRIA)
    tela.blit(titulo, (largura_tela // 2 - titulo.get_width() // 2 + rng.randint(-shake, shake), y - 44 + rng.randint(-shake, shake)))

    nucleo = (largura_tela // 2, altura_tela // 2 + 28)
    for i in range(6):
        fase = (progresso + i * 0.14) % 1.0
        raio = int(70 + fase * REGISTRADOR_ANEL_PULSO_MAX)
        alpha_anel = int(150 * (1.0 - fase))
        pygame.draw.circle(tela, (*COR_CONDUTORA_FRIA, alpha_anel), nucleo, raio, 2)

    lock_points = (0.52, 0.66, REGISTRADOR_ROLETAGEM_TRAVAR_EM)
    for i in range(3):
        x = base_x + i * (reel_w + gap)
        travado = progresso >= lock_points[i]
        rect = pygame.Rect(x + rng.randint(-shake, shake), y + rng.randint(-shake, shake), reel_w, reel_h)
        pygame.draw.rect(tela, (5, 12, 24), rect, border_radius=10)
        pygame.draw.rect(tela, COR_LOGICO_OK if travado else (COR_CONDUTORA_FRIA if i == 1 else COR_CONDUTORA), rect, 3 if travado else 2, border_radius=10)
        pygame.draw.rect(tela, (255, 255, 255, 22), rect.inflate(-10, -10), 1, border_radius=7)
        if not travado:
            idx = (agora // max(55, 120 - i * 18) + i * 2) % len(opcoes)
            texto = opcoes[idx]
            offset = int(math.sin(agora * 0.047 + i) * 17)
        else:
            texto = combo[i] if i < len(combo) else "?"
            offset = 0
        cor = COR_LOGICO_OK if travado else (COR_LOGICO_ERRO if i == 1 and progresso < 0.35 else COR_CONDUTORA_CLARA)
        render = fonte_reel.render(texto, True, cor)
        tela.blit(render, render.get_rect(center=(rect.centerx, rect.centery + offset)))
        if travado:
            pygame.draw.circle(tela, COR_LOGICO_OK, rect.center, int(54 + 8 * math.sin(agora * 0.015)), 2)
        legenda = "TRAVADO" if travado else ("ESCOLHA" if i == 1 else "SINAL")
    ...
    if progresso >= REGISTRADOR_ROLETAGEM_TRAVAR_EM:
        texto_final = fonte_grande.render(f"{tipo_final} {tipo_final} {tipo_final}", True, COR_LOGICO_OK)
        tela.blit(texto_final, texto_final.get_rect(center=(largura_tela // 2, y + reel_h + 74)))


def _desenhar_buff_registrador(tela, inimigos, tempo_atual, perfil, jogador_rect=None):
    estado = _estado_logico()
    buff = _registrador_buff_ativo(tempo_atual)
    pulso = estado.get("registrador_pulso") or {}
    if pulso and int(tempo_atual) >= int(pulso.get("fim_ms", 0)):
        estado["registrador_pulso"] = None
        pulso = {}
    if not buff and not pulso:
        return
    if jogador_rect is None:
        jogador_rect = pygame.Rect(tela.get_width() // 2 - 18, tela.get_height() // 2 - 24, 36, 48)
    cx, cy = jogador_rect.center
    cor = _cor_buff_registrador(buff or pulso.get("buff"))
    descricao = _descricao_buff_registrador(buff or pulso.get("buff"), pulso.get("porta"))
    inicio = int(estado.get("registrador_buff_inicio", tempo_atual))
    duracao = max(1, int(estado.get("registrador_buff_duracao", 1)))
    vida = max(0.0, min(1.0, 1.0 - (int(tempo_atual) - inicio) / float(duracao)))
    intensidade = 0.35 + 0.65 * vida
    rng = random.Random(int(tempo_atual) // 33 + hash(str(buff)) % 10000)

    if buff:
        raio_base = int(120 + 170 * intensidade)
        campo = pygame.Surface((raio_base * 2 + 8, raio_base * 2 + 8), pygame.SRCALPHA)
        for i in range(5 if perfil == "baixo" else 10):
            fase = ((tempo_atual * (0.0016 + i * 0.00011)) + i * 0.13) % 1.0
            raio = int(28 + fase * raio_base)
            alpha = int((115 if i % 2 else 72) * (1.0 - fase) * intensidade)
            pygame.draw.circle(campo, (*cor, alpha), (raio_base + 4, raio_base + 4), raio, 2)
        if buff in ("Q", "JK_Q", "T_Q", "RS_SET", "D_OR", "D_AND", "D_XOR"):
            for i in range(42 if perfil == "baixo" else 120):
                ang = rng.random() * math.tau
                dist = rng.randint(18, raio_base)
                px = int(raio_base + 4 + math.cos(ang) * dist)
                py = int(raio_base + 4 + math.sin(ang) * dist * 0.72)
                pygame.draw.circle(campo, (*COR_LOGICO_1, rng.randint(85, 210)), (px, py), rng.randint(1, 3))
        else:
            for i in range(28 if perfil == "baixo" else 84):
                ang = rng.random() * math.tau
                dist = rng.randint(35, raio_base)
                p1 = (int(raio_base + 4 + math.cos(ang) * dist), int(raio_base + 4 + math.sin(ang) * dist))
                p2 = (int(raio_base + 4 + math.cos(ang + 0.26) * (dist + 22)), int(raio_base + 4 + math.sin(ang + 0.26) * (dist + 22)))
                pygame.draw.line(campo, (*COR_LOGICO_0, rng.randint(60, 150)), p1, p2, 1)
        tela.blit(campo, (cx - raio_base - 4, cy - raio_base - 4))

        alvos = [alvo for alvo in inimigos or [] if _alvo_vivo(alvo)]
        alvos.sort(key=lambda alvo: math.hypot(alvo["rect"].centerx - cx, alvo["rect"].centery - cy))
        for idx, alvo in enumerate(alvos[:8 if perfil == "baixo" else 16]):
            ar = alvo["rect"]
            largura = 2 + (idx % 3)
            pygame.draw.line(tela, (5, 12, 24), (cx, cy), ar.center, largura + 3)
            pygame.draw.line(tela, cor, (cx, cy), ar.center, largura)
            if idx % 2 == 0:
                mid = ((cx + ar.centerx) // 2, (cy + ar.centery) // 2)
                pygame.draw.circle(tela, COR_CONDUTORA_CLARA, mid, 4 + idx % 4)

    pulso_fim = int(pulso.get("fim_ms", 0))
    if pulso and int(tempo_atual) < pulso_fim:
        inicio_pulso = int(pulso.get("inicio_ms", tempo_atual))
        t = max(0.0, min(1.0, (int(tempo_atual) - inicio_pulso) / max(1, pulso_fim - inicio_pulso)))
        raio = int(42 + t * 460)
        alpha = int(220 * (1.0 - t))
        pygame.draw.circle(tela, (*cor, alpha), (cx, cy), raio, 4)
        pygame.draw.circle(tela, (*COR_CONDUTORA_CLARA, max(0, alpha - 40)), (cx, cy), max(8, raio // 3), 2)
        fonte = _fonte(22, True)
        texto = str(pulso.get("texto") or descricao)
        render = fonte.render(texto, True, cor)
        _texto_contorno(tela, fonte, texto, (cx - render.get_width() // 2, cy - 94), cor)

    if buff:
        fonte = _fonte(15, True)
        texto = descricao
        render = fonte.render(texto, True, cor)
        fundo = pygame.Rect(cx - render.get_width() // 2 - 10, cy + jogador_rect.height // 2 + 16, render.get_width() + 20, 24)
        pygame.draw.rect(tela, (3, 10, 20), fundo, border_radius=7)
        pygame.draw.rect(tela, cor, fundo, 1, border_radius=7)
        tela.blit(render, (fundo.x + 10, fundo.y + 4))
...
def desenhar_circuitos(tela, inimigos, tempo_atual, config_graficos=None, manifestacao=None, jogador_rect=None):
    ...
    _desenhar_buff_registrador(tela, inimigos, tempo_atual, perfil, jogador_rect)
