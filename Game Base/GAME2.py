...
def trigger_kamikaze_explosion(inimigo):
    ...
    if dist_to_player <= 120:
        ...
        if imune_tempo_restante > 0:
            ...
            vida -= dmg_taken
            Variaveis.registrar_dano_recebido_player(efeitos_texto, dmg_taken, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, pygame.time.get_ticks())
            piscando_vida = True
            tempo_ultimo_hit_inimigo = pygame.time.get_ticks()
        ...
        while running:
            ...
            for inimigo in inimigos_comum:
                ...
                for disparo in disparos:
                    ...
                    if (
                        ...
                        dano = novas_aureas.aplicar_dano_em_alvo(
                            estados_novas_aureas, aurea, tempo_atual,
                            inimigo, dano, disparo, efeitos_texto, inimigos_comum
                        )
            ...
            for morto_novas in mortos_novas:
                if morto_novas in inimigos_comum:
                    gerar_fragmentos_morte(morto_novas, 2)
                    Variaveis.tentar_soltar_carta(morto_novas["rect"].center, tempo_atual, Chance_Sorte, inimigos_eliminados)
            ...
            boss_info = {
                ...
                "vida_maxima": vida_maxima_boss2,
                "estado_laceracao": globals().setdefault("boss2_laceracao_estado", {}),
                "atingido_por_onda": boss_atingido_por_onda,
                "hit_flag": False
            }
            if boss_info["vivo"] and boss_info["rect"] is not None:
                vida_boss2, _ = lacerante_manifestacao.atualizar_laceracao_boss(
                    boss_info["estado_laceracao"], tempo_atual, vida_boss2, vida_maxima_boss2,
                    efeitos_texto, pos_x_chefe2 + chefe_largura2 // 2, pos_y_chefe2 - 44
                )
            ...
            for inimigo in inimigos_comum:
                ...
                if inimigo.get("elite", False):
                    elite_size = (inimigo["rect"].width, inimigo["rect"].height)
                    elite_key = (id(img_base), elite_size)
                    if inimigo.get("_elite_img_key") != elite_key:
                        inimigo["_elite_img_key"] = elite_key
                        inimigo["_elite_img_cache"] = pygame.transform.scale(img_base, elite_size)
                    inimigo_image = inimigo.get("_elite_img_cache", img_base)
                else:
                ...
                # Draw Elite Aura
                if inimigo.get("elite", False):
                    glow_size = (inimigo["rect"].width + 16, inimigo["rect"].height + 16)
                    if inimigo.get("_elite_glow_size") != glow_size:
                        inimigo["_elite_glow_size"] = glow_size
                        glow_surf = pygame.Surface(glow_size, pygame.SRCALPHA)
                        pygame.draw.ellipse(glow_surf, (0, 191, 255, 60), (0, 0, glow_size[0], glow_size[1]))
                        inimigo["_elite_glow_cache"] = glow_surf
                    glow_surf = inimigo.get("_elite_glow_cache")
                ...
                if (
                    ...
                    if not personagem_imovel:
                        ...
                        if absorver_dano_devota_atual():
                            ...
                            Variaveis.registrar_dano_recebido_player(efeitos_texto, Dano_pos_resistencia_person_longe, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, tempo_atual)
                            eliminacoes_consecutivas = 0
                            bonus_pontuacao = 0
                            ...
                            if p["y"] < p["ty"]:
                                ...
                                if dist_to_player <= 50:
                                    if absorver_dano_devota_atual():
                                        ...
                                        dano_recebido_boss2 = int(vida_maxima - vida) * 0.10 + 20 + dano_boss2
                                        vida -= dano_recebido_boss2
                                        Variaveis.registrar_dano_recebido_player(efeitos_texto, dano_recebido_boss2, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, pygame.time.get_ticks())
                                        tempo_ultimo_atingido = pygame.time.get_ticks()
                                        personagem_imovel = True
                                        piscando_vida = True
                    ...
                    if ataque_vertical_ativo or ataque_horizontal_ativo:
                        for wave in ondas_nevasca:
                            ...
                            if rect_wave.colliderect(personagem_rect):
                                if absorver_dano_devota_atual():
                                    pass
                                elif imune_tempo_restante <= 0 and pygame.time.get_ticks() - tempo_ultimo_atingido >= 800:
                                    dano_recebido_boss2 = int(vida_maxima * 0.08) + 20 + dano_boss2
                                    vida -= dano_recebido_boss2
                                    Variaveis.registrar_dano_recebido_player(efeitos_texto, dano_recebido_boss2, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, pygame.time.get_ticks())
                                    tempo_ultimo_atingido = pygame.time.get_ticks()
                                    personagem_imovel = True
                    ...
                    if r_press and not boss_entrada_ativa and acertou_boss_disparo:
                        if disparo.get("tipo_manifestacao") == "lacerante_corte":
                            lacerante_manifestacao.aplicar_laceracao_boss(
                                globals().setdefault("boss2_laceracao_estado", {}),
                                tempo_atual,
                                vida_maxima_boss2,
                            )
                        ...
                        dano = novas_aureas.aplicar_dano_boss(
                            ...
                            efeitos_texto, (pos_x_chefe2 + chefe_largura2 // 2, pos_y_chefe2 - 32)
                        )
