...
personagem_paths = {
    ...
    'stop': ["Sprites/Geo1.png", "Sprites/Geo2.png", "Sprites/Geo3.png", "Sprites/Geo4.png"],
...
tempo_animacao_stop_agressivo_min = 470
janela_respiracao_agressiva_ms = 2200
...
def intervalo_animacao_stop_dinamico(tempo_atual=None, ultimo_teleporte_ms=0, ultimo_disparo_ms=0, disparo_preparando=False):
    agora = int(tempo_atual if tempo_atual is not None else pygame.time.get_ticks())
    base = int(tempo_animacao_stop)
    minimo = int(tempo_animacao_stop_agressivo_min)
    janela = max(1, int(janela_respiracao_agressiva_ms))

    def pulso_recente(ultimo_ms, peso):
        if not ultimo_ms:
            return 0.0
        idade = max(0, agora - int(ultimo_ms))
        if idade >= janela:
            return 0.0
        return float(peso) * (1.0 - idade / float(janela))

    intensidade = 0.0
    intensidade += pulso_recente(ultimo_disparo_ms, 0.50)
    intensidade += pulso_recente(ultimo_teleporte_ms, 0.62)
    if disparo_preparando:
        intensidade += 0.18
    if ultimo_disparo_ms and ultimo_teleporte_ms and abs(int(ultimo_disparo_ms) - int(ultimo_teleporte_ms)) <= 1200:
        intensidade += 0.18
    intensidade = max(0.0, min(1.0, intensidade))
    return int(round(base - (base - minimo) * intensidade))
...
LOJA_FORCADA_AVISO_MS = int(LOJA_FORCADA_AVISO_SEG * 1000)
LOJA_FORCADA_REABRIR_MS = 1200
...
aviso_loja_forcada_cartas = 0
loja_forcada_bloqueada_ate_cair_limiar = False
ultimo_forcar_loja_ms = 0
...
def calcular_cartas_compraveis(pontuacao_atual, custo_base, custo_por_carta, total_cartas_compradas):

    max_cartas = 0
    total_custo = 0
    temp_cartas_compradas = max(0, int(total_cartas_compradas or 0))
    pontos = max(0, int(pontuacao_atual or 0))

    while True:

        proximo_custo = int(custo_base + (temp_cartas_compradas * custo_por_carta))

        if proximo_custo <= 0:

            break

        if total_custo + proximo_custo <= pontos:
    ...
    return max_cartas, total_custo


def registrar_dano_recebido_player(efeitos_texto_lista, dano, x, y, tempo_atual_ms=None, origem=""):

    if efeitos_texto_lista is None:

        return

    try:

        valor = int(max(1, round(float(dano))))

    except Exception:

        return

    tempo = int(tempo_atual_ms if tempo_atual_ms is not None else pygame.time.get_ticks())

    prefixo = "-" if not origem else f"{origem} -"

    registrar_efeito_texto(
        efeitos_texto_lista,
        f"{prefixo}{valor}",
        int(x),
        int(y),
        tempo,
        (255, 72, 86),
        chave=("dano-player", int(x) // 18, int(y) // 18, valor),
        intervalo_ms=45,
        tamanho=34,
        duracao=950,
    )
...
def cancelar_aviso_loja_forcada():
    ...
    global aviso_loja_forcada_inicio_ms, aviso_loja_forcada_fim_ms, aviso_loja_forcada_cartas
...
def _cartas_compraveis_por_custo_atual(pontuacao_atual, custo_carta_atual):

    custo_base = max(1, int(globals().get("custo_base_carta", 500)))

    custo_passo = max(0, int(globals().get("custo_por_carta", 90)))

    custo_atual = max(1, int(custo_carta_atual or custo_base))

    total_cartas = max(0, int(round((custo_atual - custo_base) / max(1, custo_passo)))) if custo_passo else 0

    qtd, total_custo = calcular_cartas_compraveis(pontuacao_atual, custo_base, custo_passo, total_cartas)

    return qtd, total_custo


def loja_forcada_deve_abrir(pontuacao_atual, custo_carta_atual, tempo_atual_ms):

    global aviso_loja_forcada_inicio_ms, aviso_loja_forcada_fim_ms, aviso_loja_forcada_cartas
    global loja_forcada_bloqueada_ate_cair_limiar, ultimo_forcar_loja_ms
    ...
    cartas_compraveis, _ = _cartas_compraveis_por_custo_atual(pontuacao_atual, custo_carta_atual)

    if cartas_compraveis < max(1, LOJA_FORCADA_CARTAS_MINIMAS):
    ...
    if loja_forcada_bloqueada_ate_cair_limiar:

        return False

    tempo_atual_ms = int(tempo_atual_ms)

    if aviso_loja_forcada_inicio_ms is None or aviso_loja_forcada_fim_ms is None:

        aviso_loja_forcada_inicio_ms = tempo_atual_ms

        aviso_loja_forcada_fim_ms = tempo_atual_ms + LOJA_FORCADA_AVISO_MS

        aviso_loja_forcada_cartas = cartas_compraveis

        return False

    aviso_loja_forcada_cartas = max(aviso_loja_forcada_cartas, cartas_compraveis)

    if tempo_atual_ms < aviso_loja_forcada_fim_ms:

        return False

    ultimo_forcar_loja_ms = int(tempo_atual_ms)

    loja_forcada_bloqueada_ate_cair_limiar = True
...
def desenhar_aviso_loja_forcada(tela, tempo_atual_ms):
    ...
    if aviso_loja_forcada_fim_ms is None or not loja_forcada_ativa():

        return

    restante_ms = max(0, int(aviso_loja_forcada_fim_ms) - int(tempo_atual_ms))

    restante_seg = int(math.ceil(restante_ms / 1000.0))

    largura, altura = tela.get_size()

    fonte_titulo = pygame.font.Font(None, 34)

    fonte_timer = pygame.font.Font(None, 46)

    texto_titulo = f"LOJA FORCADA EM {restante_seg}s"

    texto_info = f"Pontos acumulados para {max(LOJA_FORCADA_CARTAS_MINIMAS, aviso_loja_forcada_cartas)} cartas"

    surf_titulo = fonte_titulo.render(texto_titulo, True, (255, 235, 150))

    surf_timer = fonte_timer.render(texto_info, True, (255, 255, 255))

    w = max(surf_titulo.get_width(), surf_timer.get_width()) + 42

    h = 86

    x = (largura - w) // 2

    y = max(26, int(altura * 0.11))

    painel = pygame.Surface((w, h), pygame.SRCALPHA)

    pygame.draw.rect(painel, (10, 8, 20, 208), painel.get_rect(), border_radius=10)

    pygame.draw.rect(painel, (255, 210, 80, 190), painel.get_rect(), width=2, border_radius=10)

    progresso = 1.0 - (restante_ms / float(max(1, LOJA_FORCADA_AVISO_MS)))

    pygame.draw.rect(painel, (65, 38, 75), (20, h - 15, w - 40, 6), border_radius=3)

    pygame.draw.rect(painel, (255, 205, 65), (20, h - 15, int((w - 40) * max(0.0, min(1.0, progresso))), 6), border_radius=3)

    tela.blit(painel, (x, y))

    tela.blit(surf_titulo, (x + (w - surf_titulo.get_width()) // 2, y + 13))

    tela.blit(surf_timer, (x + (w - surf_timer.get_width()) // 2, y + 43))
...
def reset_game_session():
    ...
    global ultimo_spawn_larapio_normal_ms, ultimo_spawn_larapio_hard_ms, larapios_pontos, loja_forcada_bloqueada_ate_cair_limiar
