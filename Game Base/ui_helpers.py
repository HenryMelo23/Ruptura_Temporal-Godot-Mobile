...
def _cartas_compraveis_hud(pontuacao_exib, custo_carta_atual):
    try:
        import Variaveis
        custo_base = max(1, int(getattr(Variaveis, "custo_base_carta", 500)))
        custo_passo = max(0, int(getattr(Variaveis, "custo_por_carta", 90)))
        custo_atual = max(1, int(custo_carta_atual))
        total_cartas = max(0, int(round((custo_atual - custo_base) / max(1, custo_passo)))) if custo_passo else 0
        qtd, _ = Variaveis.calcular_cartas_compraveis(pontuacao_exib, custo_base, custo_passo, total_cartas)
        return qtd
    except Exception:
        return int(max(0, pontuacao_exib) // max(1, custo_carta_atual))
...
def get_cached_scale(surface, tamanho, smooth=False):
    if not surface:
        return surface
    tamanho = (max(1, int(tamanho[0])), max(1, int(tamanho[1])))
    key = (id(surface), tamanho, bool(smooth))
    cached = _scale_cache.get(key)
    if cached is not None:
        return cached
    if len(_scale_cache) > 160:
        _scale_cache.clear()
    func = pygame.transform.smoothscale if smooth else pygame.transform.scale
    scaled = func(surface, tamanho)
...
def _desenhar_hud_molduras(display):
    ...
    if left.width >= 120:
        ...
        if hasattr(Variaveis, "imagem_relogio"):
            relogio = get_cached_scale(Variaveis.imagem_relogio, (raio, raio), smooth=True)
    ...
    if right.width >= 120:
        ...
        if not modo_drops:
            ...
            cartas = _cartas_compraveis_hud(hud["pontuacao_exib"], hud["custo_carta_atual"])
            _texto_contorno(display, font_peq, f"CARTAS x{cartas}", (255, 235, 150), (x0, 112))
...
def desenhar_hud_fase(
    ...
    if Variaveis.obter_modo_cartas() != "drops":
        ...
        cartas_compraveis = _cartas_compraveis_hud(pontuacao_exib, custo_carta_atual)
        texto_cartas = fonte_vida.render(f'cartas x{cartas_compraveis}', True, (255, 235, 150))
        texto_cartas_borda = fonte_vida.render(f'cartas x{cartas_compraveis}', True, (0, 0, 0))
        pos_cartas = (Variaveis.largura_mapa * 0.075 + texto_pontuacao.get_width() + 12, Variaveis.altura_mapa * 0.118 + 2)
        for dx, dy in ((-1, -1), (1, -1), (-1, 1), (1, 1)):
            tela.blit(texto_cartas_borda, (pos_cartas[0] + dx, pos_cartas[1] + dy))
        tela.blit(texto_cartas, pos_cartas)
...
def desenhar_hud_widescreen(tela_real, vida, vida_maxima, pontuacao_exib, custo_carta_atual, pontuacao_magia, cooldowns, dispositivo_ativo, eliminacoes_consecutivas, bonus_pontuacao, aurea, escudo_devota_ativo, fase_nome, cronometro_str):
    ...
    if hasattr(Variaveis, 'imagem_relogio'):
        img_rel = get_cached_scale(Variaveis.imagem_relogio, (50, 50))
    ...
    cartas_compraveis = _cartas_compraveis_hud(pontuacao_exib, custo_carta_atual)
    txt_cartas = font_pequena.render(f"CARTAS x{cartas_compraveis}", True, (255, 235, 150))
    surf_dir.blit(txt_cartas, (20, 166))
    ...
    for nome, tecla, icone_pronto, icone_recarga, cd in habilidades_lista:
        ...
        img_hab = get_cached_scale(icone, (48, 48))
