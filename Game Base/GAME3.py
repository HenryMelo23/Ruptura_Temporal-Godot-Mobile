...
def desenhar_inimigo(tela, inimigo):
    ...
    def imagem_escalada_tintada(nome_cache, tint_cor):
        cache_key = (nome_cache, id(inimigo["image"]), largura, altura, tint_cor)
        if inimigo.get("_render_cache_key") != cache_key:
            imagem_cache = pygame.transform.smoothscale(inimigo["image"], (largura, altura))
            if tint_cor:
                tint = pygame.Surface((largura, altura), pygame.SRCALPHA)
                tint.fill(tint_cor)
                imagem_cache = imagem_cache.copy()
                imagem_cache.blit(tint, (0, 0), special_flags=pygame.BLEND_RGBA_ADD)
            inimigo["_render_cache_key"] = cache_key
            inimigo["_render_cache_img"] = imagem_cache
        return inimigo.get("_render_cache_img", inimigo["image"])

    if inimigo.get("tipo") == TIPO_INIMIGO_GUARDIAO_SUCATA:
        imagem = imagem_escalada_tintada("guardiao", (55, 48, 38, 35))
        ...
        imagem = imagem_escalada_tintada("incensario", (70, 35, 115, 55))
        tela.blit(imagem, (pos_x, pos_y))
        ...
        imagem = imagem_escalada_tintada("devoto", (30, 150, 45, 70))
        tela.blit(imagem, (pos_x, pos_y))
    else:
        ...
        while running:
            ...
            for morto_novas in mortos_novas:
                if morto_novas in inimigos_comum:
                    gerar_fragmentos_morte(morto_novas, 3)
            ...
            boss_info = {
                ...
                "vida_maxima": vida_maxima_boss3,
                "estado_laceracao": globals().setdefault("boss3_laceracao_estado", {}),
                "atingido_por_onda": globals().get("boss_atingido_por_onda", {}).get("boss", 0),
                "hit_flag": False
            }
            if boss_info["vivo"] and boss_info["rect"] is not None:
                vida_boss3, _ = lacerante_manifestacao.atualizar_laceracao_boss(
                    boss_info["estado_laceracao"], tempo_atual, vida_boss3, vida_maxima_boss3,
                    efeitos_texto, pos_x_chefe3 + chefe_largura3 // 2, pos_y_chefe3 - 44
                )
                ...
                if r_press:
                        ...
                        if acertou_boss_disparo:
                            if disparo.get("tipo_manifestacao") == "lacerante_corte":
                                lacerante_manifestacao.aplicar_laceracao_boss(
                                    globals().setdefault("boss3_laceracao_estado", {}),
                                    tempo_atual,
                                    vida_maxima_boss3,
                                )
                            ...
                            dano = novas_aureas.aplicar_dano_boss(
                                ...
                                efeitos_texto, (pos_x_chefe3 + chefe_largura3 // 2, pos_y_chefe3 - 32)
                            )
                    ...
                    for disparo in disparos_boss3:
                        ...
                        if rect_disparo_boss.colliderect(rect_personagem):
                            Frascos.play()
                            if imune_tempo_restante <= 0:
                                dano_recebido_boss3 = int(vida_maxima*0.25)+450
                                vida -= dano_recebido_boss3
                                Variaveis.registrar_dano_recebido_player(efeitos_texto, dano_recebido_boss3, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, tempo_atual)
            ...
            if abrir_loja_manual and multiplayer_coop.modo_multiplayer():
                if pontuacao_exib >= custo_carta_atual:
                    multiplayer_coop.solicitar_acao("loja", 3)
                abrir_loja_manual = False
            if multiplayer_coop.acao_confirmada("loja", 3):
