...
LACERACAO_TICK_NORMAL_MS = 1050
LACERACAO_TICK_ABERTO_MS = 820
LACERACAO_DANO_POR_STACK = 0.0038
LACERACAO_DANO_ABERTO = 0.014
BOSS_LACERACAO_DURACAO_MS = 5200
BOSS_LACERACAO_TICK_MS = 950
BOSS_LACERACAO_DANO_POR_STACK = 0.0014
BOSS_LACERACAO_DANO_MAX_TICK = 0.006
...
def dano_extra_movimento(inimigo, tempo_atual):
    ...
    intervalo_tick = LACERACAO_TICK_ABERTO_MS if estado.get("aberto") else LACERACAO_TICK_NORMAL_MS
    ...
    percentual = LACERACAO_DANO_POR_STACK * stacks
    if estado.get("aberto"):
        percentual = LACERACAO_DANO_ABERTO
    dano = max(1, int(vida_base * percentual))
    inimigo["vida"] -= dano
    return dano


def aplicar_laceracao_boss(estado_boss, tempo_atual, vida_maxima_boss):
    if estado_boss is None:
        return 0
    estado = estado_boss.setdefault("laceracao_temporal", {"stacks": 0, "ultimo_tick": 0})
    stacks = min(LACERACAO_MAX_STACKS, int(estado.get("stacks", 0)) + 1)
    estado["stacks"] = stacks
    estado["expira_ms"] = int(tempo_atual) + BOSS_LACERACAO_DURACAO_MS
    estado["vida_maxima_ref"] = max(1.0, float(vida_maxima_boss or 1))
    return stacks


def atualizar_laceracao_boss(estado_boss, tempo_atual, vida_boss, vida_maxima_boss, efeitos_texto=None, x=0, y=0):
    if estado_boss is None:
        return vida_boss, 0
    estado = estado_boss.get("laceracao_temporal")
    if not estado:
        return vida_boss, 0
    if tempo_atual > int(estado.get("expira_ms", 0)):
        estado_boss.pop("laceracao_temporal", None)
        return vida_boss, 0
    if tempo_atual - int(estado.get("ultimo_tick", 0)) < BOSS_LACERACAO_TICK_MS:
        return vida_boss, 0
    estado["ultimo_tick"] = int(tempo_atual)
    stacks = max(1, min(LACERACAO_MAX_STACKS, int(estado.get("stacks", 1))))
    vida_ref = max(1.0, float(vida_maxima_boss or estado.get("vida_maxima_ref", 1)))
    percentual = min(BOSS_LACERACAO_DANO_MAX_TICK, BOSS_LACERACAO_DANO_POR_STACK * stacks)
    dano = max(1, int(vida_ref * percentual))
    vida_boss = max(0, vida_boss - dano)
    if efeitos_texto is not None:
        efeitos_texto.append({
            "texto": f"SANGRA -{int(dano)}",
            "x": int(x),
            "y": int(y),
...
def processar_fenda(fenda, inimigos, boss_info, tempo_atual):
    ...
    if boss_info and boss_info.get("vivo") and boss_info.get("rect"):
        ...
        if acertou_boss:
            ...
            aplicar_laceracao_boss(boss_info.setdefault("estado_laceracao", {}), tempo_atual, boss_info.get("vida_maxima", 1))
    return mortos
