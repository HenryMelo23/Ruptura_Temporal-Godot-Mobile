...
LARAPIO_MULTIPLICADOR_VIDA = 3.2
LARAPIO_MITIGACAO_BASE = 0.62
LARAPIO_RESISTENCIA_MAX = 0.42
LARAPIO_MULTIPLICADOR_DANO = 0.28
LARAPIO_DANO_MAX_VIDA_PCT = 0.035
LARAPIO_DANO_ESPECIAL_PCT = 0.035
LARAPIO_STUN_MS = 900
LARAPIO_ESCAPE_CRITICO_VIDA_PCT = 0.50
LARAPIO_PORTAL_ESCAPE_MS = 10000
LARAPIO_PORTAL_PAUSA_HIT_MS = 2000
    ...
    try:
        ...
        def obter_mitigacao_dano(inimigo):
            if inimigo.get("tipo", 1) == TIPO_LARAPIO:
                resistencia = min(LARAPIO_RESISTENCIA_MAX, bonus_larapio(inimigo)["resistencia"])
                return max(0.45, LARAPIO_MITIGACAO_BASE - resistencia)
        ...
        def registrar_hit_larapio(inimigo):
            agora_hit = pygame.time.get_ticks()
            inimigo["ultimo_tempo_atingido"] = agora_hit
            inimigo["portal_pausa_ate_ms"] = agora_hit + LARAPIO_PORTAL_PAUSA_HIT_MS

        def criar_orbe_cura_inimigo(posicao, fracao_vida_perdida, origem):
            orbes_cura_inimigos.append({
                "x": float(posicao[0]),
                "y": float(posicao[1]),
                "fracao": float(fracao_vida_perdida),
                "origem": origem,
        ...
        def atualizar_e_desenhar_orbes_cura_inimigos(tela):
            global vida
            if not orbes_cura_inimigos:
                return
            agora_ms = pygame.time.get_ticks()
            rect_player = pygame.Rect(pos_x_personagem, pos_y_personagem, largura_personagem, altura_personagem)
            vivos = []
            for orbe in orbes_cura_inimigos:
                idade = agora_ms - int(orbe.get("inicio_ms", agora_ms))
                if idade > int(orbe.get("duracao_ms", 0)):
                    continue
                pulso = (math.sin(agora_ms * 0.007 + orbe.get("fase", 0.0)) + 1.0) * 0.5
                x = int(orbe["x"])
                y = int(orbe["y"] - 7 + math.sin(agora_ms * 0.004 + orbe.get("fase", 0.0)) * 5)
                raio = int(9 + pulso * 4)
                rect_orbe = pygame.Rect(x - raio, y - raio, raio * 2, raio * 2)
                if rect_player.colliderect(rect_orbe):
                    vida_perdida = max(0, vida_maxima - vida)
                    cura = max(1, int(vida_perdida * float(orbe.get("fracao", 0.08)))) if vida_perdida > 0 else 0
                    if cura > 0:
                    ...
                    continue
                pygame.draw.circle(tela, (24, 88, 44), (x, y), raio + 4)
                pygame.draw.circle(tela, (92, 255, 135), (x, y), raio, 2)
                pygame.draw.circle(tela, (190, 255, 205), (x - 2, y - 2), max(3, raio // 2))
                vivos.append(orbe)
            orbes_cura_inimigos[:] = vivos
        ...
        def processar_morte_inimigo(inimigo):
            ...
            if inimigo.get("tipo", 1) == TIPO_LARAPIO:
                criar_orbe_cura_inimigo(posicao_inimigo, 0.08, "LARAPIO")

            if inimigo.get("tipo", 1) == TIPO_CURATER:
                criar_orbe_cura_inimigo(posicao_inimigo, 0.12, "CURATER")
                vida_perdida = max(0, vida_maxima - vida)
        ...
        def criar_inimigo(x, y, tipo=1):
            ...
            if tipo == 2:  # Aglomerador
                ...
                hp = vida_inimigo_maxima * LARAPIO_MULTIPLICADOR_VIDA * escala_dificuldade
        ...
        def soltar_moeda(posicao):
            ...
            if random.random() < chance:
                sprite_redimensionada = getattr(soltar_moeda, "_sprite_cache", None)
                if sprite_redimensionada is None:
                    sprite_redimensionada = pygame.transform.scale(sprite_moeda, (36, 36))
                    soltar_moeda._sprite_cache = sprite_redimensionada
        ...
        def soltar_pontos_larapio(posicao, quantidade_roubada):
            ...
            sprite_redimensionada = getattr(soltar_pontos_larapio, "_sprite_cache", None)
            if sprite_redimensionada is None:
                sprite_redimensionada = pygame.transform.scale(sprite_moeda, (32, 32))
                soltar_pontos_larapio._sprite_cache = sprite_redimensionada
        ...
        def bonus_larapio(inimigo):
            ...
            return {
                ...
                "dano": min(0.16, poder_saque * 0.004),
                "resistencia": min(LARAPIO_RESISTENCIA_MAX, 0.12 + poder_saque * 0.006),
            }
        ...
        def executar_ataque_larapio(inimigo):
            ...
            fator_riqueza = min(1.35, 1.0 + (pontuacao_exib / max(300.0, float(custo_carta_atual) * 4.0)) * 0.12)
            dano_base = int(((vida_maxima * 0.012) + (dano_inimigo_perto * 0.35)) * LARAPIO_MULTIPLICADOR_DANO * (1 + bonus["dano"]) * fator_riqueza)
            dano_base = min(dano_base, int(vida_maxima * LARAPIO_DANO_MAX_VIDA_PCT))
            ...
            if pontuacao_exib < 150:
                ...
                tempo_stun_jogador_fim = tempo_atual + LARAPIO_STUN_MS
                dano_especial = max(1, int(vida_maxima * LARAPIO_DANO_ESPECIAL_PCT))
                vida -= dano_especial
                Variaveis.registrar_dano_recebido_player(efeitos_texto, dano_especial, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, tempo_atual)
            ...
            if dano_final > 0:
                vida -= dano_final
                Variaveis.registrar_dano_recebido_player(efeitos_texto, dano_final, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, tempo_atual)
                if aurea == "Impulsiva":
        ...
        def atualizar_larapio(inimigo):
            ...
            vida_critica_escape = inimigo["vida"] < inimigo["vida_maxima"] * LARAPIO_ESCAPE_CRITICO_VIDA_PCT
            if vida_critica_escape:
                if not inimigo.get("escape_critico_larapio", False):
                    inimigo["inicio_fuga"] = tempo_atual
                    inimigo.pop("target_quina", None)
                inimigo["escape_critico_larapio"] = True
                inimigo["estado"] = "fugindo"
                inimigo["ir_direto_roubar"] = False
            else:
                inimigo["escape_critico_larapio"] = False
            ...
            pode_carregar_escape = vida_critica_escape or (coletou and menos_70_vida)
            
            if pode_carregar_escape:
                pausa_ate = int(inimigo.get("portal_pausa_ate_ms", 0))
                if tempo_atual < pausa_ate:
                    inimigo["ultimo_tick_portal"] = pausa_ate
                else:
                    ...
                    if tempo_diff > 0:
                        progresso = (tempo_diff / float(LARAPIO_PORTAL_ESCAPE_MS)) * 100.0
                        inimigo["portal_charge"] = min(100.0, inimigo.get("portal_charge", 0.0) + progresso)
                        inimigo["ultimo_tick_portal"] = tempo_atual
            else:
                inimigo["portal_charge"] = 0.0
                inimigo["ultimo_tick_portal"] = 0
                inimigo["portal_pausa_ate_ms"] = 0
            ...
            if not has_stolen and not vida_critica_escape:
            ...
            # ----------------------------------------------------
            if vida_critica_escape:
                velocidade_fuga = inimigo.get("velocidade", Velocidade_Inimigos_1) * (1.8 + bonus["velocidade"])
                if tempo_atual < inimigo.get("tempo_fuga_fantasma", 0):
        ...
        estados_novas_aureas = novas_aureas.criar_estado_novas_aureas(upgrades, pygame.time.get_ticks())
        ...
        while running:
            ...
            registrador_congelado = condutora_manifestacao.animacao_registrador_ativa(manifestacao_ativa, tempo_atual)
            if not registrador_congelado:
            ...
            if direcao_atual == 'stop':
                intervalo_stop_atual = Variaveis.intervalo_animacao_stop_dinamico(
                    tempo_atual, tempo_ultimo_dash, tempo_ultimo_disparo, disparo_preparando
                )
                if tempo_passado >= intervalo_stop_atual:
                    tempo_passado = 0
            ...
            if disparo_preparando:
                ...
                if disparo_frame_atual >= len(frames_animacao['disp']):
                    ...
                    disparo_novo = novas_aureas.marcar_disparo(estados_novas_aureas, aurea, disparo_novo, tempo_atual)
                    disparos.append(disparo_novo)
            ...
            for morto_voraz in mortos_voraz:
                if morto_voraz in inimigos_comum:
                    ...
                    inimigos_eliminados += 1
            mortos_novas = novas_aureas.atualizar(
                estados_novas_aureas, aurea, tempo_atual,
                pos_x_personagem, pos_y_personagem, largura_personagem, altura_personagem,
                inimigos_comum, efeitos_texto, dano_person_hit * fator_dano_aureas(tempo_atual),
                fator_tempo=dt
            )
            for morto_novas in mortos_novas:
                if morto_novas in inimigos_comum:
                    processar_morte_inimigo(morto_novas)
                    inimigos_comum.remove(morto_novas)
                    inimigos_eliminados += 1
            ...
            boss_info = {
                ...
                "vida_maxima": vida_maxima_boss1,
                "estado_laceracao": globals().setdefault("boss1_laceracao_estado", {}),
                "atingido_por_onda": boss_atingido_por_onda,
                "hit_flag": False
            }
            if boss_info["vivo"]:
                vida_boss, _ = lacerante_manifestacao.atualizar_laceracao_boss(
                    boss_info["estado_laceracao"], tempo_atual, vida_boss, vida_maxima_boss1,
                    efeitos_texto, pos_x_chefe + chefe_largura // 2, pos_y_chefe - 44
                )
            ...
            for disp in list(disparos_inimigos):
                ...
                fase_proj = (tempo_atual * 0.006) + disp["rect"].x * 0.035 + disp["rect"].y * 0.021
                pulso_proj = (math.sin(fase_proj) + 1.0) * 0.5
                cor_base = (
                    int(130 + pulso_proj * 58),
                    int(130 - pulso_proj * 58),
                    int(145 + pulso_proj * 92),
                )
                cor_borda = (
                    int(70 + pulso_proj * 70),
                    int(66 - pulso_proj * 44),
                    int(112 + pulso_proj * 120),
                )
                pygame.draw.circle(tela, cor_borda, disp["rect"].center, 7)
                pygame.draw.circle(tela, cor_base, disp["rect"].center, 5)
                pygame.draw.circle(tela, (238, 226, 255), (disp["rect"].centerx - 1, disp["rect"].centery - 1), 2)
                ...
                if disp["rect"].colliderect(player_rect):
                    if imune_tempo_restante <= 0:
                        ...
                        if Dano_pos_resistencia_person > 0:
                            ...
                            Variaveis.registrar_dano_recebido_player(efeitos_texto, Dano_pos_resistencia_person, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, tempo_atual)
                            tempo_ultimo_hit_inimigo = tempo_atual
            ...
            for inimigo in inimigos_comum:
                ...
                if tipo == TIPO_LARAPIO or tipo == "larapio":
                    ...
                    escape_critico = inimigo.get("escape_critico_larapio", False)
                    if portal_charge > 0.0 or escape_critico:
                        progresso_portal = max(portal_charge / 100.0, 0.16 if escape_critico else 0.0)
                        pulso_portal = 0.88 + 0.12 * math.sin(pygame.time.get_ticks() * 0.011 + inimigo["rect"].x * 0.03)
                        max_largura_portal = int(l_vis * 1.35)
                        max_altura_portal = int(a_vis * 0.45)
                        largura_portal = int(progresso_portal * max_largura_portal * pulso_portal)
                        altura_portal = int(progresso_portal * max_altura_portal * (1.0 + (1.0 - pulso_portal) * 0.6))
                        if largura_portal > 4 and altura_portal > 2:
                            ...
                            pygame.draw.ellipse(portal_surf, (18, 0, 28, 220), (0, 0, largura_portal, altura_portal))
                            pygame.draw.ellipse(portal_surf, (82, 0, 116, 210), (2, 1, max(1, largura_portal - 4), max(1, altura_portal - 2)))
                            pygame.draw.ellipse(portal_surf, (185, 47, 255, 190), (4, 2, max(1, largura_portal - 8), max(1, altura_portal - 4)), 2)
                            pygame.draw.ellipse(portal_surf, (0, 0, 0, 245), (largura_portal // 5, altura_portal // 4, max(2, largura_portal * 3 // 5), max(2, altura_portal // 2)))
                            rng_portal = random.Random(inimigo["rect"].x * 31 + inimigo["rect"].y * 17 + int(portal_charge * 10))
                            for _ in range(9):
                                p_ang = rng_portal.uniform(0, 2 * math.pi)
                                raio_x = max(2, largura_portal // 2 - 2)
                                raio_y = max(1, altura_portal // 2 - 1)
                                rx = int(largura_portal / 2 + math.cos(p_ang) * rng_portal.uniform(0.15, 1.0) * raio_x)
                                ry = int(altura_portal / 2 + math.sin(p_ang) * rng_portal.uniform(0.15, 1.0) * raio_y)
                                cor_particula = (210, 75, 255, 210) if rng_portal.random() > 0.45 else (0, 0, 0, 230)
                                pygame.draw.circle(portal_surf, cor_particula, (rx, ry), rng_portal.randint(1, 3))
                            for _ in range(5):
                                ang = rng_portal.uniform(0, 2 * math.pi)
                                x1 = int(largura_portal / 2 + math.cos(ang) * largura_portal * 0.18)
                                y1 = int(altura_portal / 2 + math.sin(ang) * altura_portal * 0.15)
                                x2 = int(largura_portal / 2 + math.cos(ang) * largura_portal * 0.48)
                                y2 = int(altura_portal / 2 + math.sin(ang) * altura_portal * 0.42)
                                pygame.draw.line(portal_surf, (126, 0, 196, 190), (x1, y1), (x2, y2), 1)
                    ...
                    if abs(dist - wave["raio"]) <= wave["largura_linha"] / 2 + max(largura_personagem, altura_personagem) / 2:
                        ...
                        if not safe and not wave["atingiu_player"]:
                            ...
                            if absorver_dano_devota_atual():
                                ...
                                dano_recebido_onda = int(dano_onda - Resistencia)
                                vida -= dano_recebido_onda
                                Variaveis.registrar_dano_recebido_player(efeitos_texto, dano_recebido_onda, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, tempo_atual)
        ...
        #AQUI GERAMOS O BOSS:
                ...
                if not boss_morte_processada:
                    for disparo in disparos[:]:
                        ...
                        if acertou_boss_disparo:
                            if disparo.get("tipo_manifestacao") == "lacerante_corte":
                                lacerante_manifestacao.aplicar_laceracao_boss(
                                    globals().setdefault("boss1_laceracao_estado", {}),
                                    tempo_atual,
                                    vida_maxima_boss1,
                                )
                            ...
                            dano = novas_aureas.aplicar_dano_boss(
                                estados_novas_aureas, aurea, tempo_atual, dano, disparo,
                                efeitos_texto, (pos_x_chefe + chefe_largura // 2, pos_y_chefe - 32)
                            )
                    ...
                    if rect_boss.colliderect(rect_personagem) and tempo_atual >= tempo_boss_entrada_fim:
                        ...
                        if tempo_atual - tempo_ultimo_ataque >= 2500:
                            ...
                            if absorver_dano_devota_atual():
                                ...
                                dano_recebido_boss = int(dano_boss_total-Resistencia)
                                vida -= dano_recebido_boss
                                Variaveis.registrar_dano_recebido_player(efeitos_texto, dano_recebido_boss, pos_x_personagem + largura_personagem // 2, pos_y_personagem - 22, tempo_atual)
                            else:
                                pass
                            Dano_person.play()
                            piscando_vida = True
            ...
            if miniboss_condutor:
                for disparo in disparos[:]:
                    ...
                    if disparo["rect"].colliderect(miniboss_condutor["rect"]):
                        ...
                        dano_condutor = novas_aureas.aplicar_dano_em_alvo(
                            estados_novas_aureas, aurea, tempo_atual,
                            miniboss_condutor, dano_condutor, disparo, efeitos_texto, inimigos_comum
                        )
            ...
            for inimigo in inimigos_comum:
                ...
                for disparo in disparos[:]:
                    ...
                    if (
                        ...
                        dano_final = novas_aureas.aplicar_dano_em_alvo(
                            estados_novas_aureas, aurea, tempo_atual,
                            inimigo, dano_final, disparo, efeitos_texto, inimigos_comum
                        )
                        if mitigacao < 1.0:
            ...
            abrir_loja_forcada = modo_loja_normal and Variaveis.loja_forcada_deve_abrir(pontuacao_exib, custo_carta_atual, tempo_atual)
            abrir_loja_manual = modo_loja_normal and (abrir_loja_forcada or Variaveis.verificar_input("Comprar na loja") or (joystick and joystick.get_button(3)))
            ...
            atualizar_e_desenhar_orbes_cura_inimigos(tela)
            ...
            novas_aureas.desenhar(tela, estados_novas_aureas, aurea, tempo_atual, largura_tela, config_graficos, player_pos=(pos_x_personagem, pos_y_personagem, largura_personagem, altura_personagem))
            Variaveis.desenhar_aviso_loja_forcada(tela, tempo_atual)
